...
$(document).ready(function () {
    var table = $('#dataTables-example').dataTable({
        ...
        "ajax": {
            "processing": true,
            "url": "https://laberp.pythonanywhere.com/admin/products?table=true",
            "dataSrc": ""
        },
    ...
    tool_bar += '<option>Được hiển thị</option>';
    tool_bar += '<option>Tất cả</option>';
    tool_bar += '<option>Đã xóa</option>';
    tool_bar += '<option>Bị khóa</option>';
    tool_bar += '</select>';
    tool_bar += '</div>';
    table.api().columns(4).search('^Được hiển thị$', true, false).draw();
