...
$(document).ready(function () {
        ...
        onAuthorize: function (data, actions) {
            return actions.payment.execute()
                .then(function () {
                    ...
                    $.ajax({
                        url: 'http://54.213.242.175:8000/merchant/purchase_service_ads',
                        method: 'POST',
    ...
    $.ajax({
        url: 'http://54.213.242.175:8000/merchant/getListAds/',
        type: 'GET',
        dataType: 'json',
        async: false,
...
$('#select_type').change(function () {
    ...
    for (var i = 0; i < arr.length; i++) {
        if (arr[i].id == this.value) {
            ...
            $.ajax({
                url: 'http://54.213.242.175:8000/merchant/getDateAvailable/' + arr[i].position + '&' +
                    arr[i].id,
                type: 'GET',
                dateType: 'json',
                success: function (data) {
