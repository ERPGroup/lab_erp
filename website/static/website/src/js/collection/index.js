$(document).ready(function () {
    ...
    url = 'http://13.67.105.209:8000/product_collection/' + id_category + '?';
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            for (var i = 0; i < response.data.length; i++) {
                ...
                html += '<a href="/shop/' + response.data[i].creator_id_id + '"><p class="name_shop text-center">SHOP ' + response.data[i].creator_id_id + '</p></a>'
                ...
                if (response.data[i].product.discount_percent != 0) {
                    html += '<span class="sales_percent">-' + response.data[i].product.discount_percent + '%</span>'
                }
                html += '</div>'
                html += '<div class="quick_view">'
                html += '<button class="btn_buy" onclick="location.href=\'http://13.67.105.209:8000/post/' + response.data[i].id + '\';">Xem thêm</button>'
                ...
                url_paginator = 'http://13.67.105.209:8000/collections/' + id_category + '?';
                if (GetURLParameter('newest') != undefined) {
...
function Newest_down() {
    ...
    var check_url = $(location).attr('pathname').split('/');
    var id_category = check_url[check_url.length - 1]
    url = 'http://13.67.105.209:8000/collections/' + id_category;
    if (GetURLParameter('page') == undefined) {
        url += '?newest=true&'
    } else {
...
function quick_view(id_post) {
    ...
    $.ajax({
        url: 'http://13.67.105.209:8000/post_data/' + id_post,
        method: 'GET',
        contentType: 'application/json',
        success: function (response) {
            console.log(response)
...
function buy_product(){
    ...
    $.ajax({
        url: 'http://13.67.105.209:8000/add_qty/' + product + '/' + quantity,
        method: 'GET',
        success: function (response) {
            if (response == -2) {
                ...
                if (confirm("Bạn muốn tiếp tục mua hàng?")) {
                    ...
                    $('#over').remove()
                    $.ajax({
                        url: 'http://13.67.105.209:8000/count',
                        method: 'GET',
                        success: function (response) {
                            $('#cart_mobi').empty()
